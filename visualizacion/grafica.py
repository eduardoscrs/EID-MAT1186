...
def graficar_parabola(h, k, p, orientacion, foco, directriz):
    ...
    plt.plot(
        ...
        marker="x",
        color="green",
        label=f"Foco ({foco[0]:.2f}, {foco[1]:.2f})",
    )
    ...
    if orientacion == "Vertical":
        plt.axhline(
            ...
            color="purple",
            linestyle="-.",
            label=f"Directriz y={directriz:.2f}",
        )
    else:
        plt.axvline(
            directriz,
            color="purple",
            linestyle="-.",
            label=f"Directriz x={directriz:.2f}",
        )
...
def generar_puntos_hiperbola(h, k, a, b, orientacion, puntos=500):
    ...
    if orientacion == "Horizontal":
        ...
        while x_actual <= h + largo:
            x_der.append(
                x_actual
            )  # Usamos las listas "derechas" para guardar toda la x
...
def graficar_hiperbola(h, k, a, b, orientacion, focos, vertices):
    ...
    if ramas_der:
        plt.plot(
            ...
            ramas_der[1],
            color="blue",
            label="Hipérbola" if not ramas_izq else "",
        )
