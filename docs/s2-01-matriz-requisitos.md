...
| --- | --- | --- | --- | --- |
| R-01 | Validar primero que el RUT ingresado sea real y valido con modulo 11. | Cumple | `backend/common/rut.py`, `validar_rut_paso_a_paso` | Mantener pruebas de RUT valido, invalido y de persona juridica antes de cada entrega. |
...
| R-05 | Construir la ecuacion general `Ax^2 + By^2 + Cx + Dy + E = 0` desde los digitos. | Cumple | `backend/conicas/core/ecuacion.py`, `construir_ecuacion_general` | Auditar resultados con 4 RUTs validos de persona natural. |
...
| R-15 | Graficar correctamente la conica en el plano cartesiano. | Cumple parcial | `backend/conicas/services/graficas.py`, `frontend/src/canvas` | Validar visualmente los 4 tipos de conica. |
...
| R-19 | Incorporar modulo de funciones por tramos generado desde el RUT. | Cumple backend | `backend/limites/service.py`, `backend/limites/constructor.py` | Integrar la interfaz de limites con el endpoint. |
| R-20 | Construir funcion por tramos con punto principal `a = d3`. | Cumple backend | `backend/limites/constructor.py` | Mostrar el resultado en la interfaz. |
| R-21 | Seleccionar caso por `d8 % 3`: removible, salto o infinita. | Cumple backend | `backend/limites/constructor.py`, campo `regla_seleccion` | Mostrar claramente la regla aplicada. |
| R-22 | Calcular limites laterales en puntos criticos. | Cumple backend | `backend/limites/analisis.py` | Verificar visualmente los tres casos en la interfaz. |
| R-23 | Determinar continuidad y tipo de discontinuidad. | Cumple backend | `backend/limites/continuidad.py`, campo `discontinuidad` | Mostrar conclusion y justificacion en pantalla. |
| R-24 | Mostrar tabla de valores cercanos al punto de analisis `a`. | Cumple backend | `backend/limites/tablas.py`, campo `tabla_valores` | Renderizar tabla en el modulo de limites. |
| R-25 | Graficar la funcion por tramos y evidenciar el comportamiento cerca del punto critico. | Cumple parcial | `backend/limites/graficas.py`, campo `puntos_grafica` | Integrar grafica de limites en React. |
| R-26 | Campos vacios en interfaz de limites para defensa oral. | Pendiente | `frontend/src/pages/LimitsPage` | Incorporar campos vacios para respuestas manuales durante la defensa. |
...
- El modulo de conicas cubre la mayor parte del enunciado matematico, pero debe
  auditarse con 4 RUTs validos de persona natural.
- El backend del modulo de limites ya cubre la construccion de funciones por
  tramos, limites laterales, continuidad, discontinuidad, tabla y puntos de
  grafica. La integracion visual sigue pendiente.
