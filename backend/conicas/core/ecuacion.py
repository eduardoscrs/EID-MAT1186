def calcular_variable_v(dv):
    ...
    if len(dv) != 1:
        raise ValueError("El digito verificador debe tener un solo caracter.")
    if dv == "K":
        return 10
    if dv == "0":
        return 11
...
def construir_ecuacion_general(cuerpo_rut, dv):
    """Construye la ecuacion general de segundo grado y documenta los pasos."""
    cuerpo_rut = "" if cuerpo_rut is None else str(cuerpo_rut).strip()
    if not cuerpo_rut.isdigit():
    ...
    pasos = []
    # Asegurar que el cuerpo tenga 8 digitos (rellenar con ceros a la izquierda si es necesario).
    ...
    pasos.append("Coeficientes iniciales calculados:")
    ...
    # Ajustes segun rubrica
    pasos.append("\nAplicando ajustes de la rubrica:")
    ...
    if suma_d5_d6 % 3 == 0:
        if d[6] % 2 == 0:
            B = 0
            pasos.append(
                f"- d5+d6 ({suma_d5_d6}) es multiplo de 3 y d7 ({d[6]}) es par "
                "-> Parabola vertical: B = 0"
            )
        else:
            A = 0
            pasos.append(
                f"- d5+d6 ({suma_d5_d6}) es multiplo de 3 y d7 ({d[6]}) es impar "
                "-> Parabola horizontal: A = 0"
            )
