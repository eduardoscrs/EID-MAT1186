...
def limpiar_rut(rut_ingresado):
    ...
    return str(rut_ingresado).replace(".", "").replace("-", "").replace(" ", "").upper()
...
def validar_cuerpo_dv_para_procesar(cuerpo, dv):
    ...
    if not es_valido:
        raise RutValidationError(
            "No se puede procesar el RUT porque no supera la validacion previa."
        )
...
def validar_rut_paso_a_paso(rut_limpio):
    ...
    multiplicador = 2
    pasos.append(
        "2. Multiplicando digitos de derecha a izquierda por la serie 2,3,4,5,6,7:"
    )
    ...
    for i in reversed(range(len(cuerpo))):
        ...
        suma += producto
        pasos.append(
            f"   Digito {digito} * {multiplicador} = {producto} "
            f"(Suma parcial: {suma})"
        )
    ...
    coincidencia = "Si" if es_valido else "No"
    pasos.append(
        f"7. Coincide el DV ingresado ({dv_ingresado}) "
        f"con el calculado? {coincidencia}"
    )
